document.addEventListener("DOMContentLoaded", () => {
    ...
    audioPlayers.forEach((audio, index) => {
        ...
        progressBar.max = 100;
        progressBar.value = 0;
        progressBar.classList.add("progress-bar", "w-full", "h-2", "rounded", "bg-gray-700", "appearance-none", "cursor-pointer", "md:w-auto");
        ...
        const track = trackList.children[index];
        track.querySelector(".flex.items-center.space-x-6").appendChild(progressBar);
    });
    ...
    // Update share links for each song with the provided link
    const shareLinks = document.querySelectorAll(".share-link");
    shareLinks.forEach((link, index) => {
        link.setAttribute("href", "https://cigarrete-after-sex.vercel.app/");
    });
});
